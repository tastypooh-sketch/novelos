...
import { SparklesIconOutline, SpinnerIcon, LightbulbIcon, RefreshIcon, PlusIcon, TrashIconOutline } from '../common/Icons';
        ...
        <div className="space-y-4">
            <div className="flex flex-wrap gap-4 items-end">
                <div className="flex-1 min-w-[200px]">
                    <label className="block text-xs font-semibold mb-1 opacity-70">Character A</label>
                    <select 
                ...
                <div className="flex-1 min-w-[200px]">
                    <label className="block text-xs font-semibold mb-1 opacity-70">Character B</label>
                    <select 
                        className="w-full p-2 rounded border"
                        style={{ backgroundColor: settings.backgroundColor, borderColor: settings.toolbarInputBorderColor, color: settings.textColor }}
            ...
            {relationshipAnalysis && (
                <div className="mt-6 p-4 rounded-lg space-y-4" style={{ backgroundColor: settings.backgroundColor }}>
                    <h4 className="font-semibold text-lg">Relationship Arc</h4>
                    ...
                    <div className="h-40 w-full relative border-l border-b" style={{ borderColor: settings.toolbarInputBorderColor }}>
                        ...
                        {relationshipAnalysis.dataPoints.map((point, i) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 rounded text-xs shadow-lg z-10" style={{ backgroundColor: settings.toolbarBg, border: `1px solid ${settings.toolbarInputBorderColor}` }}>
            ...
            <div className="flex-grow min-h-0 overflow-y-auto pr-2 space-y-6">
                {!hasAnyAnalysis && !isGeneratingAny && (
                    <div className="w-full h-full flex flex-col items-center justify-center text-center p-8 opacity-60">
                        <p>No analysis generated yet. Click "Run Full Analysis" to brainstorm structure, characters, and plot opportunities.</p>
                    </div>
                )}

                {/* Section 1: Pacing & Structure */}
                <div className="p-4 rounded-lg shadow-sm" style={{ backgroundColor: settings.toolbarButtonBg }}>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold">Pacing & Structure</h3>
                        {pacingAndStructureAnalysis && (
                            <button onClick={onRegeneratePacingAndStructure} disabled={isGeneratingPacingAndStructure} className="p-1.5 rounded hover:bg-black/10">
                    ...
                    {isGeneratingPacingAndStructure && !pacingAndStructureAnalysis ? (
                        <div className="flex justify-center p-4"><SpinnerIcon /></div>
                    ) : pacingAndStructureAnalysis ? (
                        <div className="space-y-4">
                            <PlotTimeline plotPoints={pacingAndStructureAnalysis.plotPoints} chapters={chapters} settings={settings} />
                            <div className="p-3 rounded" style={{ backgroundColor: settings.backgroundColor }}>
                ...
                <div className="p-4 rounded-lg shadow-sm" style={{ backgroundColor: settings.toolbarButtonBg }}>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold">Character Arcs & Consistency</h3>
                        {characterAnalysis && (
                            <button onClick={onRegenerateCharacters} disabled={isGeneratingCharacters} className="p-1.5 rounded hover:bg-black/10">
                ...
                <div className="p-4 rounded-lg shadow-sm" style={{ backgroundColor: settings.toolbarButtonBg }}>
                    <h3 className="text-lg font-semibold mb-4">Relationship Mapper</h3>
                ...
                <div className="p-4 rounded-lg shadow-sm" style={{ backgroundColor: settings.toolbarButtonBg }}>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold">Opportunity Detector (Plot Holes & Twists)</h3>
                        {opportunityAnalysis && (
                            <button onClick={onRegenerateOpportunities} disabled={isGeneratingOpportunities} className="p-1.5 rounded hover:bg-black/10">
