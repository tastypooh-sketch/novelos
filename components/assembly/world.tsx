...
// FIX: Added 'useEffect' to the import list from 'react' to resolve 'Cannot find name 'useEffect'' error.
...
import { SparklesIconOutline, SpinnerIcon, TrashIconOutline, ChevronDownIcon, ChevronUpIcon, MapIcon, MapPinIcon, PlusIcon } from '../common/Icons';
        ...
        <div className="h-full flex flex-col relative">
            ...
            >
             ...
             <div className="absolute top-4 left-4 flex flex-col gap-2">
                <button onClick={() => setMapState({ zoom: Math.min(5, mapState.zoom * 1.2) })} className="p-2 rounded-md" style={{backgroundColor: settings.toolbarButtonBg}}><PlusIcon/></button>
                <button onClick={() => setMapState({ zoom: Math.max(0.2, mapState.zoom / 1.2) })} className="p-2 rounded-md" style={{backgroundColor: settings.toolbarButtonBg}}><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" /></svg></button>
            </div>
             ...
             <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs opacity-70 bg-black/30 px-3 py-1 rounded-full">
                ...
                <div className="mt-4 space-y-4 border-t pt-4" style={{borderColor: settings.toolbarInputBorderColor}}>
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={handleRefine} disabled={isGenerating || !rawNotes.trim()} className="flex items-center gap-2 px-3 py-1.5 text-sm rounded-md text-white disabled:opacity-60" style={{backgroundColor: settings.accentColor}}>
                         ...
                         <button onClick={handleConsolidate} disabled={true} className="flex items-center gap-2 px-3 py-1.5 text-sm rounded-md disabled:opacity-60" style={{backgroundColor: settings.toolbarButtonBg}}>
                            <SparklesIconOutline />
                            Consolidate
                        </button>
                    </div>
        ...
        <div className="h-full flex flex-col" style={{ backgroundColor: `${settings.toolbarButtonBg}60`}}>
            <div className="flex-shrink-0 p-3 border-b flex items-center justify-center" style={{ backgroundColor: settings.toolbarBg, borderColor: settings.toolbarInputBorderColor }}>
                <div className="flex items-center gap-1 p-0.5 rounded-md" style={{backgroundColor: settings.toolbarButtonBg}}>
                    <button onClick={() => setView('crucible')} className={`px-4 py-1.5 rounded-md text-sm ${worldPanelView === 'crucible' ? 'text-white' : ''}`} style={{backgroundColor: worldPanelView === 'crucible' ? settings.accentColor : 'transparent'}}>Crucible</button>
                    <button onClick={() => setView('repository')} className={`px-4 py-1.5 rounded-md text-sm ${worldPanelView === 'repository' ? 'text-white' : ''}`} style={{backgroundColor: worldPanelView === 'repository' ? settings.accentColor : 'transparent'}}>Repository</button>
                    <button onClick={() => setView('map')} className={`px-4 py-1.5 rounded-md text-sm flex items-center gap-2 ${worldPanelView === 'map' ? 'text-white' : ''}`} style={{backgroundColor: worldPanelView === 'map' ? settings.accentColor : 'transparent'}}>
                        <MapIcon /> Map Builder
                    </button>
                </div>
            </div>
            ...
            {worldPanelView === 'crucible' ? (
                <div className="flex-grow min-h-0 p-4 flex flex-col gap-4">
                    <div className="flex-shrink-0 flex flex-col gap-2">
                        <h2 className="text-xl font-bold">The Alchemist's Crucible</h2>
                        <div
                ...
                <div className="flex-grow min-h-0 overflow-x-auto p-4">
                    <div className="flex gap-4 h-full">
                        {itemTypes.map(type => (
                            <div key={type} className="w-80 h-full flex-shrink-0 flex flex-col gap-3 p-3 rounded-lg" style={{backgroundColor: settings.toolbarBg}}>
                                <h3 className="font-semibold uppercase tracking-wider text-sm text-center opacity-80">{type}s</h3>
                                <div className="flex-grow overflow-y-auto space-y-3 pr-2 -mr-2">
                                    ...
                                    {(!groupedItems[type] || groupedItems[type]?.length === 0) && (
                                        <div className="text-center text-xs opacity-50 pt-4">No items of this type yet.</div>
                                    )}
                                </div>
                            </div>
                        ))}
